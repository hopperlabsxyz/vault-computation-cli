...
const computeCommand = program.command('compute')
    ...
    .option('-d, --otp-deals <string>', 'Config file for OTP-deals')
    ...
    .action(async (args, options) => {
        ...
        let otpDeals: Record<number, Record<Address, Record<Address, bigint>>> = {}

        if (options.otpDeals) {
            const otpData = fs.readFileSync(options.otpDeals, { encoding: 'utf8', flag: 'r' }).split('\n');
  ...
  Events : ${events.length}\n\n`);
            ...
            if (otpDeals[vault.chainId] && otpDeals[vault.chainId][vault.address]) {
                for (const [address, deal] of Object.entries(otpDeals[vault.chainId][vault.address])) {
        ...
        const csv = convertToCSV(results, {displayCashback: !!options.otpDeals});
