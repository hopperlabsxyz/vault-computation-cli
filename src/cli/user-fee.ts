...
export function setUserFeeCommand(command: Command) {
 ...
 This command require precise block input. The fromBlock and the toBlock must correspond to totalAssets updates blockNumber."
    ...
    .argument("chainId:VaultAddress", "", parseArguments)
    .requiredOption(
      "-f, --from-block <number>",
    ...
    .option(
      "-d, --deals <string>",
      "Path to the csv file containing OTC (Over-The-Counter) deals on the fee rebate. The amount of % is express in 10^2. For example, 8% is express 800 in the csv file. A fee rebate means that the user will be refunded a percentage of the fees he paid.",
      parseRebateDeals
    )
    .option(
      "--referrals <string>",
      "Path to the csv file containing OTC (Over-The-Counter) referrals on the fee rewards. The amount of % is express in 10^2. For example, 8% is express 800 in the csv file. Those referrals will overwrite the on-chain referrals. A fee reward means that the user will be rewarded a percentage of the fees generated by the referred user.",
